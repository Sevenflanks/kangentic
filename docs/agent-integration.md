...
| `getInjectionSequence?(spec)` | `(SettingsChangeSpec) => string[]` | Translate a column-level settings change (model / effort) into writes the `TerminalSubmitScheduler` may push onto the live PTY. Sibling of `getExitSequence` - both return `string[]` of writes. Claude returns `['/model X', '/effort Y']` for changed fields. Automated task and column paths suppress model writes and respawn concrete model changes; adapters without a supported live effort slash return `[]` and the caller respawns. |
...
| Gemini CLI | `~/.gemini/tmp/<projectSlug>/chats/session-<id>.json` **or** `.jsonl` | Directory scan with polling. `<projectSlug>` comes from the matching `~/.gemini/projects.json` cwd mapping, falling back to the lowercased basename only for missing, malformed, or unmatched registries. Both extensions are live (Gemini cut over on 2026-04-28); an anchored `\.json$` matches nothing on a current install |
...
- fresh preparation 會同時輸出 `OPENCODE_TUI_CONFIG` 與 `KANGENTIC_OPENCODE_TUI_CONFIG_OWNER`，兩者都指向同一個 session-private config path。只有 parent process 的兩個值完全相同時，Kangentic 才將它視為上一層 private config，允許 nested fresh startup。PTY boundary 先移除 inherited pair，再把本次 explicit pair 傳給 OpenCode child，讓下一層 Kangentic 仍能辨識來源。缺少或不相同的 marker 一律保留 fail-closed，避免靜默覆蓋使用者明示的 `OPENCODE_TUI_CONFIG`。使用者可將設定移到 OpenCode 正常的 global 或 project `tui.json` / `tui.jsonc` discovery，移除 inherited environment variable 後再啟動。
- `prepareInitialPrompt` 寫入私有 payload；fresh TUI plugin claim 並刪除後建立 session、navigate、最後 `promptAsync`。server activity plugin 觀察 `session.created` 寫 telemetry。帶 prompt 的 resume payload 由 activity plugin 驗證 native ID、reconcile 一個 `session_start` 後呼叫 `promptAsync`。App relaunch 的 promptless resume 不建立 payload，改由 per-spawn `KANGENTIC_OPENCODE_RESUME_SESSION_ID` 觸發相同驗證與 telemetry reconciliation，但不提交 prompt。驗證期間若 runtime 先回報不同 native ID，該 observed ID 取代 requested ID，避免 stale identity 覆寫 live root。fresh 不使用 `tui.session.select`，因為 mounted route API 才是 session selection 的同步生命週期點。
...
| Gemini | `~/.gemini/projects.json` key, `.project_root` markers under `tmp/`+`history/<slug>/`, and `~/.gemini/trustedFolders.json` keys; slug dirs renamed opportunistically on a basename change. | When the new-basename slug is already taken, the old slug is kept. Gemini and Kangentic resolve it through the registry mapping. |
