...
OpenCode 的初始 prompt 由 adapter 私有 payload 交付。fresh launch 會以私有 0600 TUI config 載入 `kangentic-startup.mjs`；mounted TUI plugin 建立 session 後同步 `route.navigate('session', { sessionID })`，才用 v2 client 提交 prompt。這避免 `tui.session.select` 在 route subscriber 尚未掛載時遺失。resume 維持 project-local `.opencode/plugins/kangentic-activity.js` 的 server-plugin bootstrap。帶 prompt 的 resume 使用私有 payload；app relaunch 的 promptless resume 改由 per-spawn native ID env 啟動 bootstrap，不建立 payload。OpenCode 本機 discovery 掃描 `.ts` 與 `.js`，因此 activity 安裝檔使用 `.js`；來源與 packaged build asset 維持 `.mjs`。

fresh TUI plugin 會 claim 並刪除 payload、建立 native session、navigate 後呼叫 `promptAsync`，不會退回 PTY bracketed-paste；activity plugin 從 `session.created` 取得 telemetry。resume activity plugin 用 `session.get` 驗證 native ID，再 reconcile 一個 `session_start`。只有 payload resume 會接著送出 prompt；promptless relaunch 不送 prompt。驗證期間收到不同 native root ID 時，runtime observation 成為 authoritative identity，且舊 validation 結果不得覆寫它。每條 failure path 至多 best-effort 附加一筆 sanitized error 與 private native boundary；prompt failure 前可能已有 `session_start`，start append failure 可由後續 matching native event 補寫。錯誤 telemetry 不含 raw data，SDK call 沒有 retry 或 prompt fallback；identity reconciliation 可在 requested ID 驗證期間改驗 newly observed ID。successful bootstrap `session_start` reconciliation 的 exactly once 僅指 telemetry，不代表 prompt 或 live command exactly once。Fresh session 帶完整 Task XML；resume session 只帶目前的 `resumePrompt`；沒有 prompt 的 resume 不會重送 Task XML。
