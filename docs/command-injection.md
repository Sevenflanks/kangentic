...
OpenCode's initial content follows a separate private plugin path, locally source-verified against OpenCode v1.18.4. `prepareInitialPrompt` writes an adapter-prepared private payload which the plugin claims and deletes in the same OpenCode process. For a fresh payload, the plugin creates the session, records `session.create().data.id`, writes a synthetic `session_start`, publishes `tui.session.select` through `client.tui.publish`, then sends the prompt with `promptAsync`. For a resumed payload, it validates the known ID with `session.get`, writes the synthetic `session_start`, and sends the prompt with `promptAsync` without publishing a selection. Each create, get, publish, and prompt request carries object-internal `throwOnError: true`. A bootstrap failure emits one JSONL line with the sanitized public `idle` error and its private native error boundary sharing one timestamp. Fresh real OpenCode QA remains pending.
...
For OpenCode, the queued fresh-spawn burst waits until the initial free-form content job completes, then sends only the latest burst directly with `sendCtrlC: false`. Separately, trailing live lane commands wait for same-process native-idle admission and cancel on user input before submitting through the allowed PTY transport with `sendCtrlC: false`; neither path replaces the plugin-delivered initial prompt.
