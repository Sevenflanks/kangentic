...
27. **Legacy `handoff_context` migration description**: this originally described cross-agent context packaging. The current setting enables native-history reference handoff only when the source agent changes, a prior session exists, and the destination lane opts in. It does not package a transcript, git diff, or metrics.
28. **`model_override` and `effort_override` columns on swimlanes and tasks** -- adds per-column model and effort/reasoning level overrides. Both default to NULL (inherit agent default). Read at spawn time by `prepare-spawn.ts` to set `--model` / `--effort` CLI flags. Concrete model changes suspend and respawn the session; only concrete effort changes supported by the adapter are live-injected via adapter-specific slash injection (`getInjectionSequence`). The same migration block also adds `model_override` and `effort_override` columns to the **tasks** table for per-task overrides set via the ContextBar popover. Per-task values take precedence over the swimlane override; NULL falls through to the swimlane and ultimately to the agent default.
