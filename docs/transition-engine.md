...
OpenCode 的初始內容使用 adapter 準備的私有 payload。fresh launch 以私有 `OPENCODE_TUI_CONFIG` 載入 packaged `kangentic-startup.mjs`；TUI plugin 在 route API 已掛載的生命週期建立 session，直接 `route.navigate('session', { sessionID })`，再用 flat v2 client 呼叫 `promptAsync`。project-local `.opencode/plugins/kangentic-activity.js` 只保留 activity telemetry 與 resume bootstrap。

fresh 與 resume 使用不同 payload-path env，避免分離的 TUI/server plugin 跨 Worker 競爭同一檔案。fresh TUI plugin claim 並刪除 payload，建立 session、直接導向該 session，再提交 prompt；server activity plugin 從正常 `session.created` event 寫入 `session_start` telemetry。帶 prompt 的 resume payload 由 server plugin 驗證 native session，reconcile 一個 `session_start` 後呼叫 `promptAsync`。App relaunch 的 promptless resume 不建立 payload，由 per-spawn native ID env 啟動驗證與 telemetry reconciliation，且不提交 prompt。若 runtime 在驗證期間回報不同 native root ID，observed ID 取代 requested ID。這些路徑都不使用 alternate-screen terminal paste，也沒有 transition action、timer readiness、polling或 prompt fallback。
