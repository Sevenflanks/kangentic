...
fresh 與 resume 使用不同 payload-path env，避免分離的 TUI/server plugin 跨 Worker 競爭同一檔案。fresh TUI plugin claim 並刪除 payload，建立 session、直接導向該 session，再提交 prompt；server activity plugin 從正常 `session.created` event 寫入 `session_start` telemetry。resume payload 仍由 server plugin 用 `session.get` 驗證已知 session，reconcile 一個 `session_start` 後呼叫 `promptAsync`。兩條路徑都不使用 alternate-screen terminal paste，也沒有 transition action、timer readiness、polling或 retry fallback。
