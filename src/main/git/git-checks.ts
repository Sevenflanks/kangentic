 ...
 * `{baseFlat}/{slug}-{shortId}`. The comparison is by PREFIX rather than
 * equality because a branch created before the dynamic MAX_PATH slug budget was
 * removed may carry a shorter, truncated slug; that truncation is still a valid
 * prefix of the full slug. Custom branches set by the user will not match.
 */
export function isAutoGeneratedBranch(branchName: string, taskId: string, taskTitle: string): boolean {
  ...
  if (branchSlug === 'task') return true; // fallback slug for an unsluggable title
