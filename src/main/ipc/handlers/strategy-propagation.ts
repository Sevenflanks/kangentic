...
export function propagateStrategyToLiveSessions(
  ...
  for (const { task, before, after, sourceName } of changes) {
    const liveSubmissionConfigurationChanged = !!before && (
      before.auto_spawn !== after?.auto_spawn
      || before.auto_command !== after?.auto_command
      || before.agent_override !== after?.agent_override
      || before.session_target !== after?.session_target
      || before.session_spawn_strategy !== after?.session_spawn_strategy
      || before.model_override !== after?.model_override
      || before.effort_override !== after?.effort_override
    );
    if (!liveSubmissionConfigurationChanged) continue;

    if (!task.session_id) continue;
    const session = context.sessionManager.getSession(task.session_id);
    if (!session || session.status !== 'running') continue;

    // A strategy edit invalidates queued bytes but must not re-run auto_command.
