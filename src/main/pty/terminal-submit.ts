...
export interface SubmitKeystrokesOptions {
  ...
  /** Legacy all-or-nothing verification contract retained for scheduler adapters. */
  strictVerification?: boolean;
  /** Legacy leading-prefix verification boundary retained for scheduler adapters. */
...
export class TerminalSubmit {
  ...
  async submitKeystrokes(
    ...
    const totalBytes = sanitized.reduce(
      (byteCount, command) => byteCount + Buffer.byteLength(command.text, 'utf8'),
      0,
    );
    ...
    try {
      ...
      for (const command of sanitized) {
        ...
        if (canVerify) {
          if (!confirmed) {
            ...
            sawFailure = true;
            console.warn(
              `[terminal-submit] ${source}: command verification failed - ` +
                `bytes=${Buffer.byteLength(command.text, 'utf8')} attempts=${MAX_SUBMIT_ATTEMPTS}`,
            );
            // trailing command 以已驗證 prefix 為前提；strict failure 必須在下一次 write 前終止整個 burst。
            if (opts.strictVerification) throw new Error('strict command verification failed');
          }
        } else {
      ...
      console.log(
        `[terminal-submit] ${source}: ${outcome} - delivered ${sanitized.length} command(s) to ` +
          `session ${sessionId.slice(0, 8)} totalBytes=${totalBytes}`,
      );
      return {
        outcome,
        unconfirmedCommands,
      ...
      console.error(
        `[terminal-submit] ${source}: keystroke delivery failed - ` +
          `commandCount=${sanitized.length} totalBytes=${totalBytes} session=${sessionId.slice(0, 8)}`,
      );
      throw caughtError;
    }
  }
