...
describe('handleMoveTask within the task\'s current column', () => {
  it('places the task at the requested slot via the dense rewrite', async () => {
    const response = await handleMoveTask({ taskId: 'uuid-c', column: 'To Do', position: 0 }, context);
  ...
  it('never dispatches onTaskMove for a same-column placement', async () => {
    ...
    await handleMoveTask({ taskId: 'uuid-c', column: 'To Do', position: 0 }, context);

    expect(context.onTaskMove).not.toHaveBeenCalled();
  });

  it('notifies the renderer with the column and the resulting order', async () => {
    await handleMoveTask({ taskId: 'uuid-d', column: 'To Do', position: 1 }, context);
  ...
  it('counts slots among the OTHER tasks, so the last slot is length - 1', async () => {
    await handleMoveTask({ taskId: 'uuid-a', column: 'To Do', position: 3 }, context);
  ...
  it('clamps a slot past the end to last rather than erroring', async () => {
    const response = await handleMoveTask({ taskId: 'uuid-a', column: 'To Do', position: 999 }, context);
  ...
  it('resolves a numeric display ID the same as a UUID', async () => {
    await handleMoveTask({ taskId: '13', column: 'To Do', position: 0 }, context);
  ...
  it('stays the long-standing no-op when no position is given', async () => {
    const response = await handleMoveTask({ taskId: 'uuid-c', column: 'To Do' }, context);
  ...
  it('reports the "to position N of" message for an MCP caller to read', async () => {
    const response = await handleMoveTask({ taskId: 'uuid-c', column: 'To Do', position: 0 }, context);
...
describe('handleMoveTask position validation', () => {
  ...
  it.each(junkPositions)('rejects %s without writing anything', async (_label, position) => {
    const response = await handleMoveTask({ taskId: 'uuid-a', column: 'Review', position }, context);
